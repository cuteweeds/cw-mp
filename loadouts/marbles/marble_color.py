...
if screentype=="OLED":
    ...
    pwm.freq(1000)
    pwm.duty_u16(65535)#mid32768 max 65535
    display.initr()
    display.rgb(True)
...
def blank():
    if screentype=="OLED":
        ...
        print(x,y,velocity_x,velocity_y)
        sleep_ms(20)
        #display.fillrect((x,y),(rect["width"],rect["height"]),TFT.BLACK)
        if velocity_x>0:
            x_origin = x
...
def invert_screen(reps):
    ...
    if screentype == "OLED":
        ...
        pass
def colorshift():
    global r,g,b,colorphase,colorphases,colorstep
...
def mode_picker(roll, pitch, ∆t):
    ...
    #print(state,x,y,"cutoff: ",(screen["width"]-rect["width"]),(screen["height"]-rect["height"]),velocity_x, velocity_y)
    ...
    if state == 3:
        marble()
    if state == 2:
        #print(x<(screen["width"]-rect["width"]))
        if velocity_x == 0:
            if (screen["width"]-rect["width"])<=x:
                ...
                #if (screen["width"]-rect["width"])/2 < x:
                #x -= 1
                velocity_x -=2
        if velocity_y == 0:
...
while True:    
    ...
    #display.fillrect((80,40),(-12,12),display.color(r,g,b))
    #pong()
    #kinect_pong()
    #marble()